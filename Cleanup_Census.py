...
def cleanup(data):
    ...
    data = data[data["Appointee Status "] != "CUSTOMER_CREATED "]
    data = data[data["Appointee Status "] != "CUSTOMER_SCHEDULED "]
    data = data[data["Appointee Status "] != "NO_SHOW "]
    data = data[data["Appointee Status "] != "CHECKED_IN "]
    data = data[data["Appointee Status "] != "WFP_REQUIRED "]
    ...
    #converts date string into a day of the week (Monday, Tuesday, etc.)
    ...
    data.drop(['Appointee Status ','PositionDescription ', 'Position Sensitivity Code ','Photo Required ',
                ...
                'Fingerprint End Time '], axis=1, inplace=True)
